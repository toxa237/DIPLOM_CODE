...
class CrossSection:
    def __init__(self, r, eps, L=0, eps0=1):
        ...
        self.N = RelativePermittivity(eps + [eps0], self.LAMDA, func=lambda x: np.real(x)**0.5)
    ...
    def calc_coef(self, lamb, A0=1):
        ...
        B[0, :] = 0
        for l in self.L:
            coef_x, coef_y = self.calc_matrix_for_coef(l, lamb)
    ...
    def calc_matrix_for_coef(self, l, lamb):
        ...
        k = 2 * np.pi / (lamb * 10e-9)

        b[0] = spherical_jn(l, self.N[0, lamb] * k * self.R[0])
        b[1] = self.N[0, lamb] * self.derivative_spherical_jn(l, self.N[0, lamb] * k, self.R[0])

        b[-2] += (1j ** l) * (2*l + 1) * spherical_jn(l, k * self.R[-1])
        b[-1] += (1j ** l) * (2*l + 1) * self.derivative_spherical_jn(l, k, self.R[-1])

        a[0, 0] = spherical_jn(l, self.N[1, lamb] * k * self.R[0])
        a[0, 1] = spherical_yn(l, self.N[1, lamb] * k * self.R[0])

        a[1, 0] = self.N[1, lamb] * self.derivative_spherical_jn(l, self.N[1, lamb] * k, self.R[0])
        a[1, 1] = self.N[1, lamb] * self.derivative_spherical_yn(l, self.N[1, lamb] * k, self.R[0])

        if self.R.shape[0] >= 2:
            c = 0
            layer = 1
            for i in range(2, 2 * self.R.shape[0], 2):
                a[i, c] = spherical_jn(l, self.N[layer, lamb] * k * self.R[layer])
                a[i, c + 1] = spherical_yn(l, self.N[layer, lamb] * k * self.R[layer])
                a[i, c + 2] = -spherical_jn(l, self.N[layer + 1, lamb] * k * self.R[layer])
                a[i, c + 3] = -spherical_yn(l, self.N[layer + 1, lamb] * k * self.R[layer])

                a[i + 1, c] = self.N[layer, lamb] * self.derivative_spherical_jn(l, self.N[layer, lamb] * k, self.R[layer])
                a[i + 1, c + 1] = self.N[layer, lamb] * self.derivative_spherical_yn(l, self.N[layer, lamb] * k, self.R[layer])
                a[i + 1, c + 2] = -self.N[layer + 1, lamb] * self.derivative_spherical_jn(l, self.N[layer + 1, lamb] * k, self.R[layer])
                a[i + 1, c + 3] = -self.N[layer + 1, lamb] * self.derivative_spherical_yn(l, self.N[layer + 1, lamb] * k, self.R[layer])
                c += 2
                layer += 1
